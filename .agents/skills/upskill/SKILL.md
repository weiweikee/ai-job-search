...
### Targeted mode
1. Use `read_url_content` or `read_browser_page` to retrieve the job posting from the URL.
...
1. **Run a search_web query** to find current, highly-rated study resources. Use queries like:
...
1. **Never fabricate resources.** Only cite resources found via actual `search_web` results. Do not invent course names, URLs, or authors.
2. **Search with the current year.** Include the year in every `search_web` query for resources so results stay fresh.
