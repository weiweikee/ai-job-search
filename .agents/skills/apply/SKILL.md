...
version: 1.0.0
description: >
  Starts the job application workflow to evaluate fit and draft/review CV and cover letter. Triggers on: /apply, apply, apply to this job.
---
...
- If `$ARGUMENTS` looks like a URL, use `read_url_content` to retrieve the job posting content.
...
- Any mention of agentic coding or AI tooling must reference **Antigravity CLI (agy)** by name
...
Use search_web and read_url_content to research:
...
2. **Apply Part B (narrative suggestions)** using judgment. These need interpretation, not mechanical replacement. Walk through every Part B category the reviewer returned and address it:
   ...
   - **Company/department-specific angles:** weave the reviewer's research into the cover letter opening or motivation paragraph. Verify every company claim via read_url_content/search_web before including it — do not trust reviewer research at face value.
